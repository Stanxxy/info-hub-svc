...
original_name="info_hub_svc"
original_urlname="info-hub-svc"
original_description="Awesome info_hub_svc created by Stanxxy" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/info_hub_svc "${name}"
...
then
    ...
    echo "then run 'info_hub_svc' or 'python -m info_hub_svc'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'info_hub_svc' or 'python -m info_hub_svc' or 'poetry run info_hub_svc'"
fi
