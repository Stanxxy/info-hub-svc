...
- 🎯 Entry points to execute your program using `python -m <info_hub_svc>` or `$ info_hub_svc` with basic CLI argument parsing.
...
---
# info_hub_svc Flask Application

Awesome info_hub_svc created by Stanxxy
...
```bash
git clone https://github.com/Stanxxy/info-hub-svc info_hub_svc
cd info_hub_svc
make install
```
...
```bash
info_hub_svc create-db   # run once
info_hub_svc populate-db  # run once (optional)
info_hub_svc add-user -u admin -p 1234  # ads a user
info_hub_svc run
```
